...
const Hero = ({ onTabChange }: HeroProps) => {
  ...
  return (
    <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-[#0a0f14]">
        ...
        <div className={`transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-8 leading-tight tracking-tight pb-2 relative min-h-[1.2em]">
            <span className="relative inline-block">
              {/* Glow layer */}
              <span 
                className="absolute inset-0 text-transparent bg-clip-text blur-2xl opacity-60"
                style={{ color: '#4FD1C5' }}
              >
                {typedText}
              </span>
          ...
          <p 
            className="text-xl md:text-2xl text-white/90 max-w-3xl mx-auto mb-10 transition-all duration-500 tracking-wide"
            style={{
          ...
          >
            <MagneticButton 
              className="px-8 py-4 bg-gradient-to-r from-primary to-accent-teal text-white font-semibold rounded-xl shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 transition-shadow duration-300"
              onClick={() => onTabChange('courses')}
            >
              Explore Courses
            </MagneticButton>
            <MagneticButton 
              className="px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-semibold rounded-xl border border-white/20 hover:bg-white/20 transition-colors duration-300"
              onClick={() => onTabChange('training-samples')}
            >
              Watch Demos
            </MagneticButton>
          </div>
      ...
      <button 
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 text-white/60 hover:text-white/90 transition-colors cursor-pointer group"
        aria-label="Scroll to content"
      >
        <span className="text-xs uppercase tracking-widest font-medium">Explore</span>
        <div className="relative">
          <ChevronDown className="w-6 h-6 animate-bounce" />
          <ChevronDown className="w-6 h-6 absolute top-0 left-0 opacity-30 animate-bounce" style={{ animationDelay: '0.1s' }} />
        </div>
      </button>
